#!/usr/bin/env python3
"""
Extract diarization results from JSON and save to readable text file
"""

import json
import os

def extract_diarization_to_text(json_file, output_file):
    """Extract diarization results to readable text format"""
    
    if not os.path.exists(json_file):
        print(f"❌ JSON file not found: {json_file}")
        return
    
    print(f"📖 Reading diarization results from: {json_file}")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("🎤 MP3 DIARIZATION RESULTS\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    json_file = "mp3_chunked_diarization_results.json"
    output_file = "mp3_diarization_results.txt"
    
    extract_diarization_to_text(json_file, output_file)
